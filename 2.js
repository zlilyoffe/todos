...
function onItemDbClick(){
  //find out which item was edit
  todo = onItemEdit(todoItem);
  //render
  render(todo, getItemsByStatus(todo, selectedTab), selectedTab); 
}
//view--------------------------------------
...
function TodoItem(todoItem) {
  ...
  if (todoItem.isEditable){
    ...
    input.addEventListener('click', afterEdit);
    input.addEventListener('dblclick', onItemDbClick);
    li.appendChild(input);
    //we need to put an event that say - 'edit is comlited'
  } else {
    var label = document.createElement('label');
    label.htmlFor = getItemId(todoItem.name);
    label.innerText = todoItem.name;
    label.addEventListener('dblclick', onItemDbClick);
...
function addItemToList (todo, name){
  var newItem = {name: name, active: true, id: getItemId(name), isEditable: false};
  todo.push(newItem);
}
...
function onItemEdit(todoItem){
  todoItem.isEditable == true;
}

function afterEdit(todoItem){
